...
const AddProductForm: React.FC<AddProductFormProps> = ({
  ...
  const sizeOptionsByCategory: Record<string, string[]> = {
    tiles: ["300x300 mm", "600x600 mm", "600x1200 mm", "800x1600 mm"],
    faucets: ["1/2 inch", "3/4 inch", "Wall Mounted", "Deck Mounted"],
    showers: ["6 inch", "8 inch", "10 inch", "12 inch"],
    sinks_basins: ["16x12 inch", "18x14 inch", "20x16 inch", "24x18 inch"],
    health_faucets: ["1.0 meter hose", "1.2 meter hose", "1.5 meter hose", "2.0 meter hose"],
    accessories: ["Small", "Medium", "Large", "Set"],
  };

  const selectedCategory = formik.values.category;
  const sizeOptions = sizeOptionsByCategory[selectedCategory] || ["Standard"];
  ...
  useEffect(() => {
    if (mode === "add" && !onSubmit && sellerProduct.productCreated) {
      formik.resetForm({ values: defaultInitialValues });
    }
  }, [sellerProduct.productCreated, mode, onSubmit]);
        ...
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 12, sm: 6, lg: 2 }}>
            <FormControl
              ...
              required
            >
              <InputLabel id="sizes-label">Size / Spec</InputLabel>
              <Select
                labelId="sizes-label"
                id="sizes"
                name="sizes"
