...
export const stripePaymentSuccess = createAsyncThunk<
  ApiResponse,
  { paymentOrderId: string; sessionId: string; jwt: string },
  { rejectValue: string }
>(
  "orders/stripePaymentSuccess",
  async ({ paymentOrderId, sessionId, jwt }, { rejectWithValue }) => {
    try {
      const response = await api.get(`/api/payment/stripe/success/${paymentOrderId}`, {
        headers: { Authorization: `Bearer ${jwt}` },
      ...
      return rejectWithValue("Failed to process Stripe payment");
    }
  }
);
