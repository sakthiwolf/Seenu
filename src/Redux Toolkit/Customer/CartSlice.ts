...
import { paymentSuccess } from "./OrderSlice";
import { stripePaymentSuccess } from "./OrderSlice";
...
const cartSlice = createSlice({
  ...
  extraReducers: (builder) => {
      ...
      .addCase(deleteCartItem.fulfilled, (state, action) => {
        if (state.cart) {
          state.cart.cartItems = state.cart.cartItems.filter(
            (item: CartItem) =>
              String(item._id) !== String(action.meta.arg.cartItemId)
          );
    ...
    // When payment succeeds, backend clears the cart; keep frontend state in sync.
    builder.addCase(paymentSuccess.fulfilled, (state) => {
      if (state.cart) {
        state.cart.cartItems = [];
        state.cart.totalItem = 0;
        state.cart.totalMrpPrice = 0;
        state.cart.totalSellingPrice = 0;
        state.cart.discount = 0;
        state.cart.couponCode = null;
      }
    });

    builder.addCase(stripePaymentSuccess.fulfilled, (state) => {
      if (state.cart) {
        state.cart.cartItems = [];
        state.cart.totalItem = 0;
        state.cart.totalMrpPrice = 0;
